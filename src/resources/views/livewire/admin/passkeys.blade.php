        ...
        @if (isset($credentials) && $credentials->count())
          <div class="space-y-4">
            @foreach ($credentials as $c)
              <div class="p-4 border border-secondary-200 dark:border-dark-2 rounded-lg {{ $c->disabled_at ? 'bg-secondary-50 dark:bg-dark-1 opacity-60' : 'bg-white dark:bg-dark-0' }}">
                <div class="flex items-start justify-between gap-4">
                  <div class="flex-1">
                    <!-- Credential Info -->
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                      <div class="flex items-center gap-3">
                        <div class="flex items-center justify-center w-10 h-10 {{ $c->disabled_at ? 'bg-secondary-200 dark:bg-dark-2' : 'bg-primary-1 dark:bg-dark-2' }} rounded-lg flex-shrink-0">
                          <x-lucide-fingerprint class="w-5 h-5 {{ $c->disabled_at ? 'text-secondary-500 dark:text-secondary-600' : 'text-primary-6 dark:text-dark-7' }}" />
                        </div>
                        <div class="min-w-0">
                          <p class="font-semibold text-secondary-800 dark:text-secondary-100 truncate">
                            {{ $c->alias ?? 'Sin nombre' }}
                            @if ($c->disabled_at)
                              <span class="ml-2 text-xs font-medium text-primary-6 bg-primary-1 px-2 py-0.5 rounded whitespace-nowrap">
                                REVOCADA
                              </span>
                            @endif
                          </p>
    ...
    <div class="bg-secondary-50 dark:bg-dark-1 border border-secondary-200 dark:border-dark-2 rounded-xl p-6">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <div class="bg-amber-50 border-l-4 border-amber-400 dark:bg-dark-3 dark:border-amber-600 dark:text-secondary-300 p-3 rounded">
